...
import { Handshake, Save, Send, Trash2 } from "lucide-react";
...
import { SALES_ORDER_FIELDS, SALES_ORDER_ITEM_COLUMNS } from "@/components/forms/form-configs";
...
export function SalesOrderFormPage() {
  ...
  // A "Create Sales Order" action elsewhere may route here with an unsaved,
  // server-mapped document in router state — same convention as Purchase Invoice.
  const prefill = (location.state as { prefill?: Partial<SalesOrder> } | null)?.prefill;

  const { data: doc, error: docError, isLoading: docLoading, mutate } = useSalesOrder(
    isNew ? undefined : name,
  );
  const { createDoc, updateDoc, deleteDoc, loading: saving } = useSalesOrderMutations();
  ...
  const [values, setValues] = useState<Partial<SalesOrder>>({});
  ...
  useEffect(() => {
    document.title = name && !isNew ? `Sales Order — ${name}` : "Sales Order";
  }, [name, isNew]);
  ...
  const onChange = (fieldname: string, value: any) => {
    setValues((v) => {
      ...
      if (fieldname === "customer") {
        next.customer_name = customerLookup.get(value) ?? value;
      }
      return next;
    });
    setErrors((e) => {
    ...
    setItemRows((prev) =>
      prev.map((r, i) => {
        ...
        if (fieldname === "qty" || fieldname === "rate") {
          next.amount = lineAmount(next as Pick<SalesOrderItem, "qty" | "rate">);
        }
        return next;
      }),
    );
  ...
  const addItemRow = () =>
    setItemRows((prev) => [
      ...prev,
      {
        ...
        rate: 0,
        amount: 0,
        delivery_date: values.delivery_date || todayISO(),
        uom: "Nos",
        stock_uom: "Nos",
        conversion_factor: 1,
  ...
  const buildPayload = useCallback((): Record<string, unknown> => {
    ...
    const items = itemRows.map(({ __uuid, name: _rowName, owner, creation, modified, modified_by, parent, parentfield, parenttype, docstatus, idx, ...rest }, i) => {
      ...
      return {
        ...
        delivery_date: rest.delivery_date || values.delivery_date || todayISO(),
        qty,
        uom,
        stock_uom: stockUom,
    ...
    return {
      doctype: "Sales Order",
      naming_series: values.naming_series || "SAL-ORD-.YYYY.-",
      ...
      buyer_po_no: values.buyer_po_no || undefined,
      export_status: values.export_status || undefined,
      lc_proforma: values.lc_proforma || undefined,
      lc_no: values.lc_no || undefined,
      lc_date: values.lc_date || undefined,
      lc_amount: values.lc_amount || undefined,
      lc_currency: values.lc_currency || undefined,
      lc_issuing_bank: values.lc_issuing_bank || undefined,
      lc_advising_bank: values.lc_advising_bank || undefined,
      lc_expiry_date: values.lc_expiry_date || undefined,
      latest_shipment_date: values.latest_shipment_date || undefined,
      port_of_loading: values.port_of_loading || undefined,
      port_of_discharge: values.port_of_discharge || undefined,
      final_destination: values.final_destination || undefined,
      incoterm: values.incoterm || undefined,
      shipment_mode: values.shipment_mode || undefined,
      country_of_destination: values.country_of_destination || undefined,
  ...
  const persist = useCallback(async () => {
    ...
    if (!canWrite) {
      toast.error("You do not have permission to save Sales Orders");
      return;
    }
    ...
    try {
      if (isNew) {
        const created = await createDoc(payload as Partial<SalesOrder>);
        toast.success(`Sales Order ${created.name ?? ""} created`);
        notifyDataChanged();
        if (created?.name) {
          navigate(`/selling/sales-orders/${encodeURIComponent(created.name)}`);
        } else {
          navigate("/selling/sales-orders");
        }
      } else {
        const updated = await updateDoc(name!, payload as Partial<SalesOrder>);
        setValues(updated);
      ...
      console.error("Sales Order save failed", err);
  ...
  const submitDoc = useCallback(async () => {
    ...
    try {
      ...
      const full = await postCall<SalesOrder>("frappe.client.get", {
        doctype: "Sales Order",
        name: doc.name,
      });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Sales Order"
        subtitle={isNew ? "New Sales Order" : name}
        icon={<Handshake className="h-5 w-5" />}
        actions={
          <>
            <Button
              variant="outline"
              onClick={() => (isNew ? navigate("/selling/sales-orders") : navigate(-1))}
      ...
      <FrappeForm fields={SALES_ORDER_FIELDS} values={values} errors={errors} readOnly={readOnly} onChange={onChange} />

      <SectionCard
        title="Items"
        actions={!readOnly ? <Button size="sm" variant="outline" onClick={addItemRow}>+ Add Row</Button> : undefined}
      >
        <EditableChildTable
          columns={SALES_ORDER_ITEM_COLUMNS}
          rows={itemRows}
      ...
      <ConfirmDialog
        ...
        description="Submit this Sales Order? After submit it can no longer be edited (only cancelled/amended)."
        confirmLabel="Submit"
        loading={submitting}
