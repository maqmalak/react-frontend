...
const columns: ColumnDef<FollowUpRow>[] = [
  {
    key: "due_date",
    label: "Due",
    render: (r) => <span className="text-sm">{r.due_date ? formatDate(r.due_date) : "—"}</span>,
...
const config: CrmManagementConfig<FollowUpRow> = {
  ...
  doctype: "CRM Task",
  fields: ["name", "title", "priority", "status", "start_date", "due_date", "description", "modified"],
  formFields: [
  ...
  renderCard: (r) => (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">{r.due_date ? formatDate(r.due_date) : ""}</span>
      </div>
    </div>
  ),
